package user;
    ...
    try
    {
     ...
     try
     {
     ResultSet rev=ep.execSQL("select order_dt from vat.m_issue_reg_online where '"+from_no+"' between from_no and to_no");
if(rev.next())
{
...
{
 ResultSet rev1=ep.execSQL("select order_dt from vat.m_issue_reg  where '"+from_no+"' between from_no and to_no");
if(rev1.next())
{
...
dat1 = (int) ((dt5.getTime() - dts.getTime())/(1000*60*60*24));//Diff between system date and end date
     ResultSet rsb=ep.execSQL("select wbill_sno from m_issue_reg_v where wbill_sno='"+from_no+"'");
   ...
   ep.close();
    ...
    return errorCodes.size() == 0;
      ...
      Dbbean ep=new Dbbean();
    String issue_dt="",from_no="183056";
    int dat=0,dat1=0,dat2=0;
    try
    {
     ep.connect();
     ...
     try
     {
     ResultSet rev=ep.execSQL("select to_char(order_dt,'DD/MM/YYYY') from vat.m_issue_reg_online where '"+from_no+"' between from_no and to_no");
if(rev.next())
{ System.out.println("hghg"+issue_dt);
    issue_dt=rev.getString(1);
}
else if(!rev.next())
{
 ResultSet rev1=ep.execSQL("select to_char(order_dt,'DD/MM/YYYY') from vat.m_issue_reg  where '"+from_no+"' between from_no and to_no");
if(rev1.next())
{ 
    issue_dt=rev1.getString(1);System.out.println("nooono"+issue_dt);
 SimpleDateFormat df1 = new SimpleDateFormat("dd-MMM-yyyy");
String sys = df1.format(new java.util.Date());//System date
...
DateFormat df6 = new SimpleDateFormat("dd-MMM-yyyy");
java.util.Date dt5 = df6.parse(sys);//system date converted into date type
System.out.println("out"+dts);
//This function calculate the total number of days between two dates.
dat1 = (int) ((dt5.getTime() - dts.getTime())/(1000*60*60*24));//Diff between system date and end date
 System.out.println("cvgbcvn"+dat1);
}
}
