        ...
        if (tin.matches("[\\d]+") && tin.length() != 0 && username.length() != 0 && password.length() >= 8) {
            /*if (!m22.find() || !m2.find()) {*/
                try {
                    ...
                    ResultSet rs5 = ep.execSQL("select hash from login_dealer where vat_rc_no='" + tin + "' and usr_cd='" + username + "'");
                    if (rs5.next()) {
                     
                        ResultSet rs4 = ep.execSQL("select hash from login_dealer where vat_rc_no='" + tin + "' and usr_cd='" + username + "' and  acc_log='YES'");
                        if (rs4.next()) {
                            String hashed = "";
                            int flag = 0;
