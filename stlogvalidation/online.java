...
import java.text.ParseException;
    ...
    public class online {
      ...
      public static final Integer ERR_cst_rc_no_db_ONLY = new Integer(22);
 ...
 errorCodes.put("email", ERR_email_FORMAT);//System.out.println("Noo"+result1);
  ...
  //  Email Validation ends
    ...
    boolean result = m2.find();
            ...
            //else
          //  if(result){ errorCodes.put("password2", ERR_password2_INVALID);}
          ...
          //  if(vat_rc_no.matches("[\\d]+") &&  password.equals(password2) && dob.length() != 0 && answer.length() != 0 && vat_rc_no.length() == 0 && cst_rc_no.length() == 0 && password2.length() >=6)
           ...
           try{
               ep.connect();
          ResultSet rs=ep.execSQL("select vat_rc_no from regd_dealer where vat_rc_no='"+vat_rc_no+"' and cst_rc_no='"+cst_rc_no+"' and appl_for='VC'");
          ResultSet rs1=ep.execSQL("select vat_rc_no from login_dealer where vat_rc_no='"+vat_rc_no+"'");
          ResultSet rs2=ep.execSQL("select vat_rc_no from login_dealer where usr_cd='"+username+"'");

          if(!rs.next()){
          errorCodes.put("vat_rc_no",ERR_vat_rc_no_db_ONLY);
          errorCodes.put("cst_rc_no",ERR_cst_rc_no_db_ONLY);
          }
          else
          if(rs1.next()){errorCodes.put("vat_rc_no",ERR_vat_rc_no_db1_ONLY);}
          else
          if(rs2.next()){errorCodes.put("username",ERR_username_db_ONLY);}
          ep.close();
           }
           catch(Exception e)
           {
...
if(dat<0){errorCodes.put("dob",ERR_dob1_FORMAT);}
    }
